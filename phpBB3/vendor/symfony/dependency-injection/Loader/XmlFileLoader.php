...
class XmlFileLoader extends FileLoader
{
    ...
    private function parseParameters(\DOMDocument $xml)
    {
		...
		if ($xml->documentElement !== NULL) {
			...
			if ($parameters = $this->getChildren($xml->firstChild, 'parameters')) {
				$this->container->getParameterBag()->add($this->getArgumentsAsPhp($parameters[0], 'parameter'));
			}
		}
    }
    ...
    public function validateSchema(\DOMDocument $dom)
    {
		...
		if ($dom->documentElement !== NULL){
			...
			if ($element = $dom->firstChild->getAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'schemaLocation')) {
				$items = preg_split('/\s+/', $element);
				for ($i = 0, $nb = \count($items); $i < $nb; $i += 2) {
					if (!$this->container->hasExtension($items[$i])) {
						continue;
