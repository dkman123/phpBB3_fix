...
abstract class TemplateHelper
{
	...
	public static function highlightNode(DOMNode $node, $prepend, $append)
	{
		...
		elseif ($node instanceof DOMCharacterData || $node instanceof DOMProcessingInstruction)
			$node->data .= $uniqid;
		// DK ***
		if ($dom->documentElement !== NULL) $docXml = TemplateLoader::innerXML($dom->documentElement);
		else $docXml = TemplateLoader::innerXML($dom->firstChild);
...
abstract class TemplateLoader
{
	...
	{
		// DK ***
		if ($dom->documentElement !== NULL) $xml = self::innerXML($dom->documentElement);
		else $xml = self::innerXML($dom->firstChild);
	...
	protected static function loadAsHTML($template)
	{
		...
		if ($dom->documentElement !== NULL)
			...
			$xml = '<?xml version="1.0" encoding="utf-8" ?><xsl:template xmlns:xsl="' . self::XMLNS_XSL . '">' . self::innerXML($dom->firstChild->firstChild->firstChild) . '</xsl:template>';
		$useErrors = \libxml_use_internal_errors(\true);
...
class Parser extends IRProcessor
{
	...
	public function parse($template)
	{
		...
		// DK***
		if ($dom->documentElement !== NULL) $this->parseChildren($ir->documentElement, $dom->documentElement);
		else $this->parseChildren($ir->firstChild, $dom->firstChild);
...
class PHP implements RendererGenerator
{
	...
	protected function compileTemplate($template)
	{
		...
		// DK ***
		if ($ir->documentElement !== NULL) $php = $this->serializer->serialize($ir->documentElement);
		else $php = $this->serializer->serialize($ir->firstChild);
...
class TemplateChecker implements ArrayAccess, Iterator
{
	...
	public function checkTemplate($template, Tag $tag = \null)
	{
		...
		if ($dom->documentElement !== NULL) {
			...
			foreach ($this->collection as $check)
				$check->check($dom->firstChild, $tag);
		}
	}
		...
		do
		{
			...
			foreach ($this->collection as $k => $normalization)
			{
				if ($i > 0 && !empty($normalization->onlyOnce))
					continue;
				// DK ***
				if ($dom->documentElement != NULL) $normalization->normalize($dom->documentElement);
				else $normalization->normalize($dom->firstChild);
			}
